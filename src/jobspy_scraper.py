...
def _normalize_jobspy_row(row, source_site: str) -> dict:
    ...
    posted_at = str(posted_val) if posted_val and str(posted_val) != 'nan' else None
...
def scrape_bayt(keywords: list[str] = None, results_per_kw: int = 50) -> int:
    ...
    try:
        ...
        logging.info('[JobSpy] Install: pip install python-jobspy')
        return 0
    ...
    for kw in kws:
        for location in LOCATIONS[:3]:   # Egypt, Cairo, Saudi Arabia
            logging.info(f'[Bayt] scraping "{kw}" in {location}...')
            try:
                df = scrape_jobs(
                    site_name=['bayt'],
                    search_term=kw,
                ...
                logging.info(f'[Bayt] "{kw}" / {location}: {len(df)} found')
                time.sleep(2)

            except Exception as e:
                logging.info(f'[Bayt] error for "{kw}" / {location}: {e}')
                time.sleep(3)
...
def scrape_indeed_arab(keywords: list[str] = None, results_per_kw: int = 50) -> int:
    ...
    for kw in kws:
        for location, country_code in INDEED_COUNTRIES.items():
            logging.info(f'[Indeed] scraping "{kw}" in {location}...')
            try:
                df = scrape_jobs(
                    site_name=['indeed'],
                    search_term=kw,
                ...
                logging.info(f'[Indeed] "{kw}" / {location}: {len(df)} found')
                time.sleep(2)

            except Exception as e:
                logging.info(f'[Indeed] error: {e}')
                time.sleep(3)
...
def scrape_google_jobs(keywords: list[str] = None, results_per_kw: int = 50) -> int:
    ...
    for kw in kws:
        for location in ['Egypt', 'Cairo Egypt', 'Saudi Arabia']:
            logging.info(f'[Google Jobs] scraping "{kw}" in {location}...')
            try:
                df = scrape_jobs(
                    site_name=['google'],
                    search_term=kw,
                ...
                logging.info(f'[Google Jobs] "{kw}" / {location}: {len(df)} found')
                time.sleep(2)

            except Exception as e:
                logging.info(f'[Google Jobs] error: {e}')
                time.sleep(3)
...
def run(keywords: list[str] = None) -> int:
    ...
    logging.info(f'[JobSpy] total saved: {total}')
    return total
