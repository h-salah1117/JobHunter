...
def scrape_linkedin_jobs(
    ...
    if SCRAPER_MODE == 'production':
        logging.info('[LinkedIn] Production mode detected — skipping to avoid IP ban on HuggingFace.')
        logging.info('[LinkedIn] Run locally with SCRAPER_MODE=local to collect data.')
        return []
    ...
    for page in range(pages):
        ...
        try:
            ...
            logging.info(f'[LinkedIn] request error page {page}: {e}')
            break
        ...
        if not cards:
            logging.info(f'[LinkedIn] No cards found on page {page} — possible block or selector change.')
            break
        ...
        logging.info(f'[LinkedIn] page {page+1}: {len(cards)} cards found')
...
def run(keywords: str = 'data scientist', location: str = 'Egypt', pages: int = 3):
    ...
    logging.info(f'[LinkedIn] saved {saved}/{len(jobs)} jobs.')
    return saved
