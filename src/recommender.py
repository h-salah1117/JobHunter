...
def load_job_skill_matrix() -> tuple[pd.DataFrame, list[str], list[str]]:
    """Returns (matrix_df, job_ids, skill_names) - cached in memory."""
    global _MATRIX_CACHE
    now = time.time()
    # Cache for 60 seconds
    if _MATRIX_CACHE['pivot'] is not None and (now - _MATRIX_CACHE['last_loaded']) < 60:
        return _MATRIX_CACHE['pivot'], _MATRIX_CACHE['job_ids'], _MATRIX_CACHE['skill_names']
    ...
    _MATRIX_CACHE = {
        'pivot': pivot,
        'job_ids': list(pivot.index.astype(str)),
        'skill_names': list(pivot.columns),
        'last_loaded': now
    }
    return pivot, _MATRIX_CACHE['job_ids'], _MATRIX_CACHE['skill_names']
